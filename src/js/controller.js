console.log(
  'SASS is a beeer way of writting CSS which has some nice additional features which makes writing CSS in a large scale application a lot easier. Browsers dont understand Sass so it has to be converted to CSS. Parcel will gonna do it for us'
);
console.log(
  "Start:index.html is our entry point/ We added a build script which the command that we are going to run in the end once we're already building the application. The entry point can also be a JS file, also could be in a different folder and we can specify all that in the parcel command "
);
console.log(
  'First we have to INITIALIZE a new project with Parcel: Terminal -> npm init'
);
console.log(
  'In order to run Parcel we need to INSTALLED it. Terminal npm i parcel -D (dev dependency). This command simply instal the latest version that is available on npm for parcel'
);
console.log(
  'To be able to install a better version - like the next version - we can use Terminal: npm i parcel@next -D'
);
console.log('RUN a Parcel: Terminal: npm run start ');
console.log('To install Saas. Terminal: npm install sass@1.26.10');
console.log(
  'Everything we develop is in the src folder. Only what we see in the browser is comiing from the dist folder (distribuition). That is the whole logic of having a module bundler: It takes our raw source code and compiles it into a nice package (in the dist) that is ready to ship to browsers.'
);
console.log(
  "To add Polyfills for ES6 features to our codebase, to add this special packages:  npm i core-js regenerator-runtime. Now we import them at the top with: import 'core-js/stable' . This will make sure old browsers are still supported by our application "
);
console.log(
  'This is for polyfilling ascync/await: regenerator-runtime/runtime. We could cherry pick but here we are importing everything'
);
console.log(
  "OBS: We can use npm to calculate Fraction. We can use GOOGLE to search npm packages for example npm fractional. You can see on npmjs.com. You can installed on terminal: npm install fractional. Then we need to import (we will import inside recipeView - import fractional from 'fractional') "
);
...
//////////////////////////////////////////////////////////////////////////////////////
console.log(
  '////////////////////////// CHAPTER 294 - Event Handlers in MVC: Publisher-Subcriber Pattern //////////////////////////////////  '
);
...
const controlAddBookmark = function () {
  ...
  if (!model.state.recipe.bookmarked)
    model.addBookmark(
      model.state.recipe
    ); //adding a bookmark with the current recipe
